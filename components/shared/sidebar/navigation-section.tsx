...
export function NavigationSection({ title, items }: { title: string; items: NavigationMenuSection[] }) {
  ...
  useEffect(() => {
    ...
    items.forEach((section) => {
      newOpenItems[section.title] = section.items?.some((subItem) => pathname === subItem.url || pathname.startsWith(subItem.url)) ?? false;
    });
          ...
          if (section.items && section.items.length > 0) {
            return (
              <CollapsibleSection
                ...
                setOpen={(isOpen) => setOpenItems((prev) => ({ ...prev, [section.title]: isOpen }))}
              />
            );
          }
          ...
          return (
            <SidebarMenuItem key={section.title}>
              <SidebarMenuButton asChild tooltip={section.title}>
                <span
                  className={
                    isActive ? "text-primary font-bold" : isDisabled ? "text-gray-900 opacity-50 cursor-not-allowed dark:text-gray-500" : ""
                  }>
    ...
    <>
      <SidebarMenuItem>
        <SidebarMenuButton
          ...
          <span className={section.isDisabled ? "text-gray-900 opacity-50 cursor-not-allowed dark:text-gray-500" : ""}>
